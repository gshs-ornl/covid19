...
make_dat <- function(state = NA_character_, url = NA_character_,
                     page = NA_character_, county = 'US',
                     cases = NA_integer_, update_date = NA,
                     deaths = NA, presumptive_positive = NA_integer_,
                     recovered = NA_integer_, tested = NA_integer_,
                     hospitalized = NA_integer_, contry = NA_character_,
...
convert_date <- function(date, format, tz = 'UTC') {
  ...
  if (length(date) > 1) {
    dates <- c()
    for (d in date) {
      d <- append(d, as.POSIXct(lubridate::as_datetime(date, format = format,
                                                       tz = tz)))
    }
  }
}