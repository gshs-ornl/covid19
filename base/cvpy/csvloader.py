...
class CSVLoader(object):
    ...
    def __init__(self, db, logger=logging.getLogger(ce('PY_LOGGER', 'main')), dry_run=0):
    ...
    def create_attribute(self, dataset, attr, name=f"FIXME: name autogenerated by {sys.argv[0]}", meta=None, ignore_duplicate=False):
        ...
        try:
           self.db.cur.execute(f"INSERT INTO {args.schema}.attributes(dataset_id, attr, name, meta) VALUES (%s, %s, %s, %s)", (dataset, attr, name, Json(meta) if meta else None))
    ...
    def load(self, csv_stream, op='append', logger=None, rows=[]):
        ...
        if args.op == 'replace':
            ...
            self.db.cur.execute(f"SELECT count(*) from {args.schema}.stav")
