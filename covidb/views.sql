...
create or replace view scraping.vw_country_data as
    select ctry.country,
       provider,
       region,
       u.url,
...
CREATE OR REPLACE VIEW scraping.vw_ps_country_data as
(
...
CREATE OR REPLACE VIEW scraping.vw_ps_state_data as
(
select array_to_json(array_agg(row_to_json(t
    )
    )
...
from (
         select ctry.country,
                ...
                region,
                u.url,
                sd.access_time as county_access_time,
                cases,
                updated,
                deaths,
                presumptive,
                ...
                sg.scrape_group
         from scraping.state_data sd
                  join static.country ctry ON ctry.id = sd.country_id
                  join static.states s ON s.id = sd.state_id
                  join scraping.scrape_group sg ON sg.id = sd.scrape_group_id
                  join scraping.pages p ON p.id = sd.page_id
                  join static.urls u ON u.id = sd.url_id
     ) as t
...
CREATE OR REPLACE VIEW scraping.vw_ps_county_data as
(
select array_to_json(array_agg(row_to_json(t
    )
    )
